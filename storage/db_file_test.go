...
func TestDBFile_Read(t *testing.T) {
	path := "/tmp/kvdb"
	_ = os.MkdirAll("/tmp/kvdb", os.ModePerm)
...
func TestNewDBFile(t *testing.T) {
	...
	tests := []struct {
		...
		{"f-string", args{"/tmp/kvdb", 0, FileIO, 1 * 1024 * 1024, String}, nil, false},
		{"f-list", args{"/tmp/kvdb", 0, FileIO, 1 * 1024 * 1024, List}, nil, false},
		{"f-hash", args{"/tmp/kvdb", 0, FileIO, 1 * 1024 * 1024, Hash}, nil, false},
		{"f-set", args{"/tmp/kvdb", 0, FileIO, 1 * 1024 * 1024, Set}, nil, false},
		{"f-zset", args{"/tmp/kvdb", 0, FileIO, 1 * 1024 * 1024, ZSet}, nil, false},

		{"m-string", args{"/tmp/kvdb", 1, MMap, 1 * 1024 * 1024, String}, nil, false},
		{"m-list", args{"/tmp/kvdb", 2, MMap, 2 * 1024 * 1024, List}, nil, false},
		{"m-hash", args{"/tmp/kvdb", 3, MMap, 3 * 1024 * 1024, Hash}, nil, false},
		{"m-set", args{"/tmp/kvdb", 4, MMap, 4 * 1024 * 1024, Set}, nil, false},
		{"m-zset", args{"/tmp/kvdb", 5, MMap, 5 * 1024 * 1024, ZSet}, nil, false},
	}
