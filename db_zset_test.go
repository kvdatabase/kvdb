...
func TestRoseDB_ZAdd(t *testing.T) {
	...
	key := []byte("my_zset")
	err := db.ZAdd(key, 310.23, []byte("kvdatabase"))
...
func TestRoseDB_ZScore(t *testing.T) {
	...
	ok, s := db.ZScore(key, []byte("kvdatabase"))
	t.Log(ok, s)
...
func TestRoseDB_ZRevRank(t *testing.T) {
	...
	t.Log(db.ZRevRank(key, []byte("kvdatabase")))
	t.Log(db.ZRevRank(key, "Java"))
	t.Log(db.ZRevRank(key, "kvdatabase"))
	assert.Equal(t, db.ZRevRank(key, "kvdatabase"), db.ZRevRank(key, []byte("kvdatabase")))
